...
## Added during the whole-book coherence and evidence pass

| Lecture-note or teaching need | Rights, evidence, and reconstruction | Destination | Teaching function and boundary |
|---|---|---|---|
| Seven context-sensitive choices used in the earlier opening chapter | A new book-native synthesis was drawn; no advertisement, website, journal figure, or third-party photograph was copied. | `figures/context-enters-choice.svg` plus PNG fallback; Chapter 2 | Groups all seven cases by representation and comparison, path to action, and social meaning or accessibility. Every case carries an evidence label, and the footer states that context sensitivity is not automatically irrationality. |
| Sensory access, attention, and partial evidence | New vector synthesis based on the lecture's cross-species examples; no protected animal photographs were reused. | `figures/sensory-windows-partial-world.svg` plus PNG fallback; Chapter 3 | Separates what a sensory system can detect from what receives attention and what guides behavior; it does not infer subjective experience from a receptor range. |
| Regression toward a conditional mean | New figure generated from a declared synthetic data-generating process. | `figures/regression-conditional-shrinkage.svg` plus PNG fallback; Chapter 15 | Makes conditional shrinkage visible while explicitly separating regression from a causal treatment effect. |
| Present bias and preference reversal | New, parameter-declared teaching comparison. | `figures/discount-model-crossover.svg` plus PNG fallback; Chapter 19 | Contrasts a quasi-hyperbolic reversal with exponential discounting using visible arithmetic; it illustrates a model implication rather than estimating a population parameter. |
| Hidden-zero and future-test demonstrations | New teaching diagrams with synthetic alternatives and observations. | `figures/hidden-zero-frame.svg` and `figures/model-underdetermination.svg` plus PNG fallbacks; Chapters 19 and 30 | The first shows how a frame redirects attention without declaring the correct choice. The second shows that observed fit can underdetermine a model and that new data can favor rather than prove an account. |
| Silence, dissent, and groupthink diagnosis | New mechanism map; no historical or journal artwork reused. | `figures/silence-mechanism-diagnostic.svg` plus PNG fallback; Chapter 28 | Distinguishes several causes of silence and maps each to a diagnostic test and safeguard; silence or a poor outcome alone does not diagnose groupthink. |
| Camp David as a two-issue negotiation illustration | New abstract issue map rather than a reproduced map, photograph, or complete historical account. | `figures/camp-david-two-issue-map.svg` plus PNG fallback; Chapter 37 | Shows how priorities across sovereignty and security can support package design while warning that the diagram omits parties, history, and implementation. |
| Random sampling, random assignment, participant flow, and publication selection | Three original methods figures; the selection figure is generated from a declared, deterministic simulation whose source data and parameters are retained. | `figures/random-sampling-vs-assignment.svg`, `figures/participant-flow-threats.svg`, and `figures/selected-literature-simulation.svg` plus PNG fallbacks; Appendices C and D | Separates generalizability from causal identification, traces where selection can alter an estimand, and demonstrates how selective visibility can exaggerate the published mean. |

These additions were chosen because each changes what a reader can understand, test, or diagnose. Decorative lecture images, protected journal panels, ambiguous screenshots, and examples already represented by a stronger book-native visual remained excluded.
...
- Source-level geometry QA passes for all 108 reader-facing SVG figures. The rendered-book pass loads all 116 HTML figure placements at desktop and phone widths and all 115 EPUB placements at 768 px and 390 px with no broken images or page overflow. The newly added and substantially revised figures were also captured and visually inspected in their source and rendered forms.
