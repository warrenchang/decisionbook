...
![Same evidence, different stories. Every panel repeats the same 32 synthetic observations on identical axes, in arbitrary units. Panel A shows the points alone; B–D add curves suggesting steady growth, acceleration, and tapering growth. The curves illustrate interpretations; they are not fitted models or claims of equal support.](../figures/model-underdetermination.svg){#fig-model-underdetermination fig-alt="Four panels contain identical scattered observations with an overall upward pattern. The first shows only points. The others add a straight rising line, a curve that rises increasingly steeply, and a curve whose rise gradually flattens. No points or axis scales change between panels." .phone-fit width=100%}

**Evidence constrains models; assumptions help select among them.** The three stories are not equally credible merely because we can draw them. We need to examine how well each accounts for the observations, what mechanism could produce its shape, and what further evidence would distinguish the alternatives. For the platform proposal, the same rising adoption figures might be presented as steady progress, gathering momentum, or early enthusiasm approaching its limit. A persuasive message should explain why its interpretation deserves more weight and what result would count against it.
...
The **foot-in-the-door technique** starts with a small request before a larger related one. Freedman and Fraser (1966) found that agreement to an initial request could increase later compliance. The **door-in-the-face technique** begins with a larger request that is rejected, followed by a smaller one; Cialdini and colleagues (1975) studied how this apparent concession can encourage reciprocal compliance. Accepting a request does not establish that someone now believes the proposal is good. [Chapter 36](36-preparing-and-claiming-value.qmd#chapter-36-start) applies the distinction between concessions and independent value to bargaining.
