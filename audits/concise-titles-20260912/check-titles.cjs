...
const updates=JSON.parse(fs.readFileSync('audits/concise-titles-20260912/changes.json','utf8'));
const records=JSON.parse(fs.readFileSync('audits/chapter-openings-20260912/changes.json','utf8')).map(r=>{const u=updates.find(u=>u.chapter===r.chapter);return u?{...r,title:u.title,kept_subtitle:'*'+u.subtitle+'*'}:r;});
const temp=process.env.OPENING_CHECK_TEMP||'/private/tmp/decision-book-concise-titles-20260912';
...
(async()=>{
 ...
 fs.writeFileSync(path.join(root,'audits/concise-titles-20260912',format+'-openings.json'),JSON.stringify(report,null,2)+'\n');
