...
const out=path.join(root,'audits/study-centered-20260924/visual-qa');fs.mkdirSync(out,{recursive:true});
const cases=[['03-attention-what-becomes-evidence','fig-laptop-distraction'],['15-samples-randomness-regression-and-calibration','fig-confidence-accuracy'],['31-why-stories-move-minds','fig-donation-information'],['39-behavior-design-make-the-better-action-easier','fig-vaccination-plans']];
...
(async()=>{
 ...
 for(const [stem,id] of cases){
  ...
  for(const width of (epubRoot ? [390,768] : [390,1280])){
   ...
   const geom=await figure.evaluate(el=>{const im=el.querySelector('img');const box=el.getBoundingClientRect(),ib=im.getBoundingClientRect();return {figureWidth:box.width,imageWidth:ib.width,naturalWidth:im.naturalWidth,imageComplete:im.complete,caption:el.querySelector('figcaption')?.innerText}});
