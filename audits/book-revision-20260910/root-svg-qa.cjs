...
(async()=>{
 ...
 const figs=JSON.parse(fs.readFileSync(path.join(dir,'inventory.json'))).figures.filter(x=>x.owner==='root'&&x.path.endsWith('.svg'));
 ...
 await browser.close();fs.writeFileSync(path.join(dir,'root-svg-text-qa.json'),JSON.stringify(results,null,2)+'\n');
