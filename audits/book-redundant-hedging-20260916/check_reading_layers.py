...
def inspect():
    ...
    for source in SOURCES:
        ...
        # Chapter 13's sole footnote repeated its preceding account of masked
        # priming. Removing its call and definition also removes Quarto's
        # automatically generated footnotes section. No authored heading or
        # incoming source link is being dropped.
        if source == 'chapters/13-accessibility-familiarity-and-ease.qmd' and 'footnotes' in missing:
            if re.search(r'^\[\^[^]]+\]:', current, re.M):
                issues.append(f'{source}: expected no footnotes after the editorial deletion')
            for incoming in SOURCES:
                if '13-accessibility-familiarity-and-ease.qmd#footnotes' in (ROOT / incoming).read_text():
                    issues.append(f'{incoming}: link to removed automatic footnotes section')
            missing.remove('footnotes')
        if missing:
    ...
    report = {
        ...
        'intentional_automatic_heading_removal': 'Chapter 13 footnotes section: its sole redundant footnote and call were removed; no authored heading or incoming source link was lost.',
