...
(async()=>{
 ...
 const out='/Users/ra25fi/Library/CloudStorage/OneDrive-AalborgUniversitet/decision_book/audits/personal-hotel-numbers-20260919/';
 const checks=[];
 for(const width of [1400,390]){
  const p=await b.newPage({viewport:{width,height:1000}});
  await p.goto('http://127.0.0.1:8836/docs/chapters/29-culture-and-identity-the-same-action-is-not-the-same-act.html',{waitUntil:'networkidle'});
  await p.addStyleTag({content:'html{scroll-behavior:auto!important;}'});
  const note=p.locator('.callout.personal-example').filter({hasText:'From my experience: the numbers hotels leave out'});
