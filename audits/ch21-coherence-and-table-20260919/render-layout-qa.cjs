...
const output = path.resolve('audits/ch21-coherence-and-table-20260919');
const ids = ['fig-habit-loop','fig-habit-formation-curve','tbl-wanting-liking','fig-reward-prediction-error-shift','fig-reward-uncertainty-comparison','tbl-20-brain','fig-urge-wave-observation'];
(async () => {
  ...
  try {
      ...
      for (const id of ids) {
        ...
        const record=await target.evaluate(n=>{
          ...
          return {id:n.id,insideCallout:!!n.closest('.callout'),width:r.width,height:r.height,
            ...
            table:table?{columns:table.querySelector('thead tr').children.length,headers:[...table.querySelector('thead tr').children].map(x=>x.textContent.trim()),rows:table.querySelectorAll('tbody tr').length,width:table.clientWidth,scrollWidth:table.scrollWidth,fontSize:getComputedStyle(table).fontSize}:null,
        ...
        if(record.table) {
          record.table.horizontalScroll=record.table.scrollWidth>record.table.width+3;
          if(record.table.horizontalScroll) throw new Error('Table requires horizontal scrolling: '+id+' at '+width);
        }
        if(id==='tbl-wanting-liking' && JSON.stringify(record.table.headers)!==JSON.stringify(['','Wanting','Liking'])) throw new Error('Incorrect table columns');
    ...
    console.log(`PASS: ${mode}, all seven floats in main text; images loaded and three-column comparison verified at both widths. Both tables fit without horizontal scrolling.`);
