...
mapping = []
for name in re.findall(r"^\s*-\s+(chapters/[^\s]+\.qmd)\s*$", (ROOT / "_quarto-html.yml").read_text(), re.M):
    title = re.sub(r"\s+\{[^}]+\}$", "", re.search(r"^# (.+)$", (ROOT / name).read_text(), re.M).group(1))
    mapping.append({"old": name, "new": name, "title": title})
