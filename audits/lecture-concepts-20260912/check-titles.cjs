...
const updates=JSON.parse(fs.readFileSync('audits/lecture-concepts-20260912/chapter-titles.json','utf8'));
const records=JSON.parse(fs.readFileSync('audits/chapter-openings-20260912/changes.json','utf8')).map(r=>{const u=updates.find(u=>u.chapter===r.chapter);return {...r,title:u.title,kept_subtitle:'*'+u.subtitle+'*'};});
const temp=process.env.OPENING_CHECK_TEMP||'/private/tmp/decision-book-lecture-concepts-20260912';
...
(async()=>{
 ...
 for(const r of records){
  ...
  const check=await page.evaluate(({expected,anchors,chapter,title})=>{
   ...
   const error=[];
   const actualTitle=norm(h1.querySelector('.chapter-title')?.textContent||h1.textContent.replace(/^\d+\s*/,''));
   if(actualTitle!==title)error.push('Chapter title mismatch: '+actualTitle);
   const side=document.querySelector('#quarto-sidebar');
   if(side){const link=[...side.querySelectorAll('a')].find(a=>a.hasAttribute('href')&&new URL(a.getAttribute('href'),location.href).pathname===location.pathname);if(!link||!norm(link.textContent).endsWith(title))error.push('Sidebar title mismatch');}
  ...
  },{expected:r.kept_subtitle.slice(1,-1),anchors:[...r.retained_opening_anchors,...(r.moved_heading?[r.moved_heading.anchor]:[])],chapter:r.chapter,title:r.title});
 ...
 fs.writeFileSync(path.join(root,'audits/lecture-concepts-20260912',format+'-openings.json'),JSON.stringify(report,null,2)+'\n');
 if(errors.length)throw Error(JSON.stringify(errors));
 for(const width of (format==='html'?[1440,390]:[768,390])){
  await page.setViewportSize({width,height:1000});
  for(const n of [10,15,42]){
