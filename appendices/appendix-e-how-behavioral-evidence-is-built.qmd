...
For a reminder study, repeated appointments could support a within-person comparison only if earlier messages do not contaminate the later contrast in an unaddressed way. Choose the unit and sequence to identify the effect, rather than choosing repeated measurement simply because it produces more rows.
...
![Random sampling supports population generalization, an aspect of external validity. Random assignment supports the internal validity of a causal comparison. Both depend on how the study is designed and carried out.](../figures/random-sampling-vs-assignment.svg){#fig-random-sampling-vs-assignment fig-alt="Two columns connect random sampling to external validity and random assignment to internal validity. Sampling selects people from a population; assignment allocates participants to conditions. The footer qualifies both inferences by design and implementation." .phone-fit}

@fig-random-sampling-vs-assignment separates the two tasks. Recruitment determines whose experience the study can describe; assignment creates the comparison used to estimate a causal effect. Attrition, treatment delivery, and spillovers can still threaten that comparison, as the following sections explain.
