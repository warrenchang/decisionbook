# How Behavioral Evidence Is Built
...
Evidence is not a result with a small *p*-value. It is a chain from a clear question to a defensible measure, comparison, estimand, analysis, and bounded claim. Randomization, statistical power, preregistration, and replication strengthen different links; none can repair a construct that was never defined or a claim the design cannot identify.
:::
...
1. **Before data:** complete the design card in @tbl-evidence-design-card.
2. **During implementation:** monitor assignment, exposure, attrition, contamination, measurement, and deviations without looking for a preferred result.
3. **After data:** state the estimand first, report uncertainty and all planned outcomes, separate confirmatory from exploratory analysis, and narrow the claim to what the design learned.
...
Figure @fig-claim-to-design-pipeline shows the discipline: each arrow carries assumptions. A transparent failure at one stage can improve the next design. An opaque success cannot.
...
Choose one claim from the book or current media. Place it in Figure @fig-claim-to-design-pipeline. State the question, construct, measure, comparison, estimand, and final sentence. Mark the weakest arrow. Then propose the smallest feasible redesign that would strengthen that link. If the redesign cannot support the original claim, narrow the claim.

EXPERIENCE – EXPLAIN – APPLY (MAXIMUM 120 WORDS)  
Experience: state the original claim and evidence. Explain: identify one design link and one assumption. Apply: give the revised design or the narrower warranted claim.
:::
