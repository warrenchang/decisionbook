...
def reading_map(number):
    ...
    return clean_svg(f'master-loop-part-{number}.svg', '\n'.join(s+['</svg>'])+'\n')
...
def decision_map():
    desc = ('An organizing map of a decision, with current context and information as inputs. '
            ...
            'construct options, predict consequences, value consequences, and choose and commit. '
    ...
    s=begin(700,893,'A decision in the making',desc)
    ...
    s.append('<rect x="44" y="70" width="612" height="813" rx="24" fill="#f7f9fc" stroke="#9fb2c3" stroke-width="2"/>')
    ...
    s.append('</g>')
    return ('\n'.join(s+['</svg>'])+'\n').replace('height="893" viewBox="0 0 700 893"', 'height="833" viewBox="0 60 700 833"', 1)
