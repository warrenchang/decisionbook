...
Every visible choice is the end of a longer process: attention selected evidence, interpretation supplied meaning, a menu made some actions possible, prediction projected consequences, and valuation assigned weight. But judging only the final selection hides omitted options, sacrificed alternatives, and the feedback that will shape the next decision. Therefore, this chapter maps decision-making as a loop so that each upstream contribution—and the quality of the process—can be inspected.
:::
...
This chapter introduces the whole navigation system: **Notice → Interpret → Construct options → Predict consequences → Value consequences → Compare and choose → Act → Learn**. Later chapters slow down one connection at a time; this chapter keeps the complete path visible.
:::
...
A marketing director must choose a product manager. The visible decision is a name on an offer letter; the hidden process begins much earlier. The committee must **notice** job-relevant evidence, **interpret** what each work sample reveals, **construct options** such as further assessment or redesigned responsibilities, **predict** future performance, **value** different contributions, **choose** and **act**, and later **learn** from performance without letting the outcome rewrite the original record.
