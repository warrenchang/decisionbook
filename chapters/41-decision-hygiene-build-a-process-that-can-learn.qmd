...
::: {#chapter-41-start .chapter-epigraph}
> “What is wanted is not the will to believe, but the wish to find out, which is its exact opposite.”
>
> — Bertrand Russell, [*Free Thought and Official Propaganda*](https://www.gutenberg.org/files/44932/44932-h/44932-h.htm), Conway Memorial Lecture, 24 March 1922
:::
...
[Chapter 2](02-building-a-better-decision-alternatives-opportunity-cost-information-and-robustness.qmd#chapter-02-start) clarified alternatives, trade-offs, and information. This chapter brings the behavioral explanations together to build a procedure that keeps those elements open to examination. We will follow what needs to be protected before a discussion, what disagreement can reveal during it, and what a later review can establish from the original record.

::: {.callout-note .core-idea icon=false}
## Core Idea

Protect judgment before the answer becomes obvious: define the criteria, gather independent assessments, examine disagreement, and record what is expected. Afterward, compare the outcome with that original record. The process should help people revise what the evidence challenges.
:::
...
A striking experiment shows why adding information can make judgment worse. Dana, Dawes, and Peterson (2013) asked students to predict other students’ semester grades using background information, including prior grades. For one target, they also conducted an unstructured interview. In one condition, the interviewee secretly began answering closed questions using a rule based on letters in the questions rather than truthful answers. Interviewers still reported obtaining useful information; their confidence did not reliably distinguish these answers from truthful ones.

Predictions made after interviews correlated less strongly with actual grades than predictions made from the background information alone: 0.31 versus 0.61 in the first study. The interviews supplied an intelligible impression that diluted useful evidence. This was a student-grade task, not a test of every employment interview. Its warning is nevertheless concrete: feeling that we understand a person can coexist with predicting that person less accurately.

A hiring process can repeatedly favor the wrong cue, or it can produce different judgments depending on which qualified panel reviews the same evidence. Both can change who gets the job, but correcting the favored cue will not necessarily reduce the variation. Before redesigning the process, distinguish the direction of error from its inconsistency. Bias has a direction relative to a defensible target. A forecaster who is consistently too optimistic or a scale that always reads two kilograms high is biased when later outcomes or a calibrated standard supplies that target. In value-laden decisions, the benchmark must be specified and defended. **Noise** is unwanted variation among repeated, commensurable judgments that should be similar. If qualified teachers grade the same essay very differently, physicians use different thresholds for the same case, or managers rate the same candidate from 4 to 9 on a ten-point scale, the system is noisy (Kahneman et al., 2021).

![Dots represent repeated judgments of the same or comparable cases; target centers represent outcomes or accepted benchmarks. The patterns distinguish systematic error from unwanted variability. Level noise concerns persistent differences in judges’ average levels; pattern noise concerns different reactions to case features. Occasion noise adds within-judge variation across occasions.](../figures/bias-and-noise.svg){#fig-bias-and-noise fig-alt="Four target diagrams compare low and high bias with low and high noise. Separate panels define level noise as persistent differences in judges' average levels and pattern noise as different reactions to case features."}

The four targets in @fig-bias-and-noise separate the location and spread of comparable judgments around an observed or accepted benchmark. **Level noise** is persistent variation in judges' average severity or generosity. **Pattern noise** concerns differences in reactions to particular cases. It has two components: **stable pattern noise**, such as a judge consistently giving unusual weight to a case feature, and **occasion noise**, variation in the same judge’s response across times or contexts. Estimate level noise, stable pattern noise, and occasion noise with a repeated-judgment design (Kahneman et al., 2021, ch. 16).
...
To investigate systematic bias, ask which recurring factor shifts the average away from the defensible target. For noise, use @tbl-36-noise to distinguish generally different severity, different weighting of case features, and variation across occasions:

| Noise component | What it looks like | First diagnostic question |
| --- | --- | --- |
| Level noise | Some judges are generally harsher or more generous. | Does the result depend on which judge was assigned? |
| Stable pattern noise | Judges persistently weight features idiosyncratically. | Are the criteria and their weights shared? |
| Occasion noise | The same judge varies with time, order, fatigue, or context. | Would this person decide the same way tomorrow? |
: Three components of noise in a judgment system {#tbl-36-noise}

**Legitimate discretion** is different: variation can reflect relevant information, values, or circumstances. Ask whether the exception can be explained and documented before treating it as an error to remove.
...
An instruction to “ignore the chair’s opinion” comes too late if members have already heard it. The process must capture their assessments earlier and preserve them for review. @fig-structured-judgment-pipeline groups that work into six stages. The checklist below expands two of them: establish a baseline before gathering judgments, and challenge the emerging conclusion before committing. This is a practical synthesis of the evidence discussed here; the later audits and record templates help carry out particular steps.

![A structured judgment workflow connects definition, decomposition, independent assessment, aggregation, recording, and review.](../figures/structured-judgment-pipeline.svg){#fig-structured-judgment-pipeline fig-alt="Six steps define a decision, separate its criteria, elicit independent judgments, compare and combine them, record a decision, and review the result. A return arrow informs the next process while preserving the original record." .phone-fit}
...
A low-stakes reversible decision may need only a quick check. A repeated, consequential, uncertain, contested, difficult-to-reverse, or partly automated decision deserves more structure.
...
A detailed project plan can make its completion date feel well grounded, yet its account of the intended path can leave delays and unexpected costs out of view. Committing resources requires a comparison with what happened in other projects. An inside view explains why this candidate, project, product, or strategy is special. An outside view begins with what happened in comparable cases and then adjusts for case-specific evidence. The reference class should be close enough to inform and broad enough to contain useful variation.

Research on project forecasts documents underestimation of costs and delays, giving this comparison practical importance (Kahneman & Lovallo, 1993; Flyvbjerg et al., 2002). The reference class is itself a judgment and can be gamed. Record alternative classes considered, the outcome distribution in each, and the reason for every adjustment so that “our case is different” becomes an inspectable claim.
...
Discussion can expose missing information and test an interpretation, but hearing others first can also change what a member reports. The chair’s enthusiasm may shift the comparison, make a reservation harder to voice, or lead several members to repeat the same reason. Capture the separate assessments before asking the group to work through them.
...
Aggregation helps when judges possess relevant information and their errors are not perfectly correlated; it cannot remove a shared bias, and it can dilute a person who holds uniquely diagnostic evidence. Hong and Page’s (2004) mathematical model shows how diverse search methods can outperform individually strong but similar problem solvers under specified conditions. That is a model-based result, not evidence that any demographically diverse team will outperform another. The practical question is whether the group’s process preserves and uses relevant differences in knowledge and approach (Sunstein & Hastie, 2015). Inspect the spread and reasons before deciding whether to average, weight, investigate, or escalate a decisive concern.
...
In repeated predictive tasks with measurable outcomes, simple mechanical or statistical rules often match or outperform unaided human judgment because they apply weights consistently (Meehl, 1954; Dawes et al., 1989; Grove et al., 2000). The target, data, and deployment context determine whether the model serves the decision.

AI is often valuable because it improves **prediction**, but prediction is only one task inside a decision. Building on Agrawal et al. (2019), @tbl-ai-prediction-judgment-causation separates forecasting from deciding what matters, what could change the outcome, and who can authorize an action.^[Agrawal et al. (2019) use *judgment* for assigning payoffs to consequences. In standard decision-science terminology, and throughout this book, that task is **valuation**. Elsewhere, **judgment** retains its broader meaning of forming beliefs and assessments.]
...
When a model materially informs a consequential decision, complete the five fields in @tbl-minimum-ai-use-record. The purpose is not to preserve every keystroke. It is to make the contribution, verification, authority, and correction path inspectable.
...
For low-stakes drafting or search assistance, a brief record may be enough. As consequence, repetition, irreversibility, unequal power, or automation grows, the record and validation should grow with it. [Appendix F](../appendices/appendix-c-portable-course-tools.qmd#ai-use-record) provides a printable version.
:::

[Chapter 42, Deciding With Data and AI](42-data-driven-decision-making.qmd#data-driven-decision-making) makes this a design opportunity: what becomes possible when prediction and some analytical assistance cost less? The process developed here supplies the conditions for using that opportunity well: compare estimates independently, state values and trade-offs, authorize an action, and preserve evidence for later review. The final chapter applies that process to machine learning, language models, and practical cases, asking when a better or cheaper estimate actually changes what it is worth doing.

Sometimes the comparison reveals that the next useful action is to gather information. A pilot, work sample, diagnostic test, or negotiation question may resolve an uncertainty before a larger commitment. Ask what could be learned, whether it could change the decision, and whether the expected benefit justifies the cost.

## During the decision: structure disagreement

Two committee members can call a candidate “too risky” for different reasons: one expects poor performance, while the other finds even a small chance of failure too costly. More debate about the candidate will go in circles until the disagreement is located. Separate prediction from valuation. Ask first what each person believes will happen and what evidence supports that belief. Ask separately what outcomes matter, which trade-offs are acceptable, and whose welfare or rights count. Many arguments become clearer when "too risky" is decomposed into probability and consequence.

[]{#challenge-the-assumptions}

Once the disagreement is clear, test whether credible differences would change the preferred option. The **sensitivity analysis** introduced in [Chapter 2](02-building-a-better-decision-alternatives-opportunity-cost-information-and-robustness.qmd#stress-test-the-recommendation) becomes a shared check on the recommendation:

- Which forecast or priority does the recommendation depend on most?
- How far would it have to change before another option becomes preferable, and is that threshold within a plausible range?
- Which assumptions could move together, and does the recommendation survive a jointly unfavorable scenario?
- If the choice is fragile, would further information be worth obtaining, or could a staged or reversible commitment reduce the cost of being wrong?

Report the conditions under which the preferred option wins. For a project, compare continuing, redesigning, pausing, and stopping from the same starting point. A limited pilot, staged investment, contingency clause, or exit option may make the decision more manageable when another forecast cannot resolve the uncertainty. Specify what evidence would justify further commitment before making it; the later review can then examine whether that condition was met.

Checklists can protect against omissions in repeated work when they are short, located at a useful pause, and tied to known failures. In Haynes and colleagues’ (2009) before-and-after study of eight hospitals, inpatient complications fell from 11.0% to 7.0% and mortality from 1.5% to 0.8% after a surgical safety program introduced a checklist. The larger rollout offers a caution: a study of checklist adoption in 101 Ontario hospitals found no statistically significant reduction in complications or mortality (Urbach et al., 2014). Neither design randomly assigned the whole implementation. A checklist’s content, use, and local consequences therefore need evaluation, rather than assuming that adoption reproduces the earlier gains. A hiring checklist likewise needs to address an actual failure, such as scoring work samples before discussing impressions.


A **premortem** asks each person to imagine that the plan has failed and independently write plausible causes. Klein (2007) proposes that imagining failure makes reservations easier to voice; that rationale is not evidence that the exercise reliably improves final decisions. A **red team** goes further by constructing an adversarial test of assumptions, evidence, and execution. Red teams need genuine access, time, and protection; a ceremonial critic who cannot change the plan is not decision hygiene. A designated devil's advocate can be dismissed as merely performing a role, so it should not replace authentic independent concern. Challenge earns its place only when someone has authority to inspect the evidence and the team records what changed in response.

These tools also depend on the [conditions for speaking up developed in Chapter 28](28-authority-groupthink-and-shared-responsibility.qmd#match-the-safeguard-to-the-missing-step). A hiring panel cannot use an independent score that its author feels compelled to withdraw after the chair speaks. Inviting specific dissent, protecting the record, and responding constructively to correction give psychological safety an operational role in the process (Edmondson, 1999).

For the distinct routes by which authority, suppressed dissent, unshared information, pluralistic ignorance, and diffused responsibility can silence challenge, see [Social Influence](28-authority-groupthink-and-shared-responsibility.qmd#chapter-28-start).
...
### Build support into meetings and decisions {#design-choices-not-just-advice}

Once the weak stage is identified, choose a change that addresses it. If members misunderstand a criterion, another independent score will not supply a shared meaning; they need examples they can compare. If concerns are understood but withheld, the reporting procedure and the response to dissent need attention. Chapters 30–40 provide tools for these different problems. Decision hygiene puts the selected tool into a process whose effect can be reviewed.

[]{#better-meetings-do-not-let-the-presentation-become-the-decision}

Many organizations call a meeting before defining what must be decided. Participants hear a polished recommendation, then try to recover independent judgment from inside its frame. The stages in @tbl-36-meeting protect independent input before a recommendation can frame the discussion, then give challenge, decision, and review their own place:
...
| Challenge | Use an outside view, premortem, strongest objection, and “what would change our minds?” | Concerns have a place in the decision and can change it. |
...
The meeting should end with a conditional recommendation: the chosen path, its main reasons, the best feasible alternative forgone, the assumptions that could reverse the choice, and the safeguards needed to proceed. In the job-offer example from Chapter 2, an agreed workload or supervision arrangement could make acceptance conditional on the support the student needs. A review date and a named person responsible for following up turn an intention to check into an assigned task. The decision-journal template below preserves these commitments for comparison with the outcome.
...
Use the six questions in @tbl-35-decision-audit to separate what was predicted, valued, and left out of the story, then identify a change to the next decision process:

| Audit focus | Audit question | Common failure | Process redesign |
| --- | --- | --- | --- |
...
The audit can reveal missing options, unsupported predictions, hidden values, invisible opportunity costs, contextual pressures, and feedback that was never designed. Its purpose is prospective: make the next decision more testable than the last one. The four cells in @fig-decision-audit keep that process assessment separate from the outcome: a favorable result can hide a weakness, while an unfavorable result can follow sound work.

![A sound or weak process can be followed by either a favorable or unfavorable outcome.](../figures/decision-audit.svg){#fig-decision-audit fig-alt="A two-by-two matrix crosses sound and weak decision processes with favorable and unfavorable outcomes. A sound process can still meet an unfavorable outcome, and a weak process can receive a favorable one." .phone-fit}

## After the outcome: compare results with the original reasoning {#after-the-outcome-learn-without-worshipping-luck}

[]{#what-a-decision-journal-is}

The hiring committee wants to learn from the appointment, but its members now remember having anticipated the problem. If the review relies on those recollections, a disappointing outcome can confirm everyone’s judgment while correcting no one’s process. Learning requires a record the outcome cannot rewrite. A **decision journal** is a collection of structured, time-stamped records created at two different moments. Before the outcome is known, an entry preserves what was knowable, expected, valued, and chosen. After a specified review date or trigger, a linked review records what happened and what should be learned **without overwriting the original entry**. Its unit is a consequential decision, not a day in the writer's life.

Meeting minutes may document the choice without preserving what was expected under the alternatives. The journal adds those forecasts and a rule for interpreting later evidence, so the review can examine more than whether the chosen option turned out well.

Several research traditions inform the journal. Decision analysis identifies the options, consequences, and values to record (Raiffa, 1968; Keeney & Raiffa, 1976). Hindsight and outcome-bias research explains why later recollection cannot substitute for a contemporary record (Fischhoff, 1975; Baron & Hershey, 1988). Forecasting research makes predictions resolvable across repeated decisions (Brier, 1950; Mellers et al., 2014).

Writing and review also depend on the conditions. In a study of Chinese judges, judges who wrote reasons before deciding were less influenced by an induced negative feeling than judges who proceeded directly to judgment; forced deliberation produced a similar effect (Liu, 2018). A meta-analysis across 46 samples with 2,136 participants found higher individual and team performance after structured debriefing than before it or in no-debrief control groups (an average standardized difference of 0.67) (Tannenbaum & Cerasoli, 2013). Accountability can also encourage conformity when the evaluator’s preferred answer is known (Lerner & Tetlock, 1999).
...
[]{#decision-journal-example}

Consider how the record constrains a later interpretation. A university teacher hopes an AI-assisted feedback tool will save editing time. An initial test suggests it can, but some drafts need major correction; faster production could come at the expense of feedback students can use. Before a pilot, the teacher must decide which results would justify adoption and which would stop it. The case and numbers in @tbl-simplified-decision-journal are hypothetical. Its two-stage record preserves those commitments so that a later time saving cannot quietly replace the original standard of success.
...
| **3. Evidence and reference class** | A local test of 20 assignments reduced average editing time from 12 to 8 minutes, but 4 drafts needed major correction. | Across 100 assignments, editing averaged 8.5 minutes, 8% needed major correction, 62% of students rated the feedback helpful, and no privacy breach was reported. |
| **4. Predictions and assumptions** | Pilot forecasts: 60% chance editing time is below 9 minutes; 70% chance at least 70% of students rate feedback helpful; 15% chance more than 10% of drafts need major correction. Assumptions: instructors catch material errors and opting out is meaningful. | Resolve the same forecasts: time below 9 minutes—yes; helpfulness at least 70%—no; correction above 10%—no. The recorded correction rate is below the threshold; unrecognized errors and whether opting out felt meaningful remain untested. |
| **5. Values and stakeholders** | Faster feedback matters, but accuracy, privacy, student trust, and a genuine option to decline are protected values. A privacy breach is unacceptable even if time is saved. | The observed major-correction rate met the threshold and no privacy breach was reported. Helpfulness was weaker than hoped. Trust and the experience of students who opted out were not measured, so those original values cannot yet be evaluated. |
...
The pilot saved time, yet failed the teacher’s stated helpfulness threshold. The record prevents one favorable result from redefining success. It also shows what remains unknown: student trust, the experience of opting out, and the alternative use of teaching-assistant time. Review each claim against the evidence that can resolve it, rather than turning the mixed outcome into a single verdict.
...
[]{#decision-journal-before-choice}

[]{#decision-journal-after-outcome}

To build this record for another decision, use the matched rows in @tbl-35-decision-journal. They identify the decision, separate prediction from valuation, and require the later review to answer the same questions recorded before choosing. Append the review; never edit the original entry into agreement with what occurred.
...
The review should end with a reasoned next step. Change a forecast, rule, or information-gathering practice when the evidence warrants it; retain a sound procedure when the unfavorable result provides no reason to replace it. Being able to explain what will change—and what will stay—makes the review useful for the next decision.

## The decision-improvement cycle: notice, test, ask, design, learn

[]{#fig-master-loop-finale}

To return to the main workflow, identify the obstacle, choose a change, and check its effect. If a review leaves the next step unclear, use the prompts in @tbl-35-1 to locate the unanswered question.
...
After the outcome, the committee reviews what was knowable at the time. Across hires, it examines calibration, predictive validity, subgroup outcomes, exceptions, and process variation. The opening choice has become an inspectable learning system.

[]{#ethics-governance-and-proportionality}

The committee still needs to test what its structure rewards. A rubric can consistently favor the wrong quality, and a model trained on past hiring can reproduce historical discrimination. Applicants need a way to challenge errors, and relevant differences between cases need reasoned, reviewable exceptions. Responsibility remains with the people authorized to make the decision, even when a model or consultant supplies advice.

Apply the same scrutiny to the controls: a checklist should support professional attention, and a decision journal should support learning without becoming surveillance. Examine whose opportunities improve and who bears additional burdens before treating a more consistent process as a better one.

[]{#proportionate-decision-process}

Match the procedure to both the stakes and the difficulty. Familiar, inexpensive, reversible choices with clear feedback usually need less structure. Novel decisions with uncertain assumptions, costly commitments, or delayed feedback need more explicit forecasts and challenge. Strategic dependence adds a reason to examine others' likely responses; effects on rights or on many people add a reason to include affected perspectives and a way to contest errors.

Use @tbl-36-proportionality to select the protection that addresses the actual difficulty. A short note may be sufficient for a low-cost reversible choice; a repeated hiring process needs comparable evidence, independent judgments, and checks for unfair treatment. Remove a control when its burden exceeds its useful contribution.
...
| Overconfident plan | Outside view and probability ranges | Sensitivity checks, premortem, jointly unfavorable scenarios |
| Costly or irreversible error | Compare reversible alternatives and exit costs | Pilot, staged commitment, safeguards, stop criteria |
...
| Inconsistent repeated predictions | Transparent statistical baseline | Validated model with fairness and drift audit |
| Uncritical reliance on AI | Independent human view and verification | Contestability, subgroup audit, human review board |
...
1. **Define:** state the decision, deadline, owner, affected people, and most costly error. List the feasible alternatives, including what happens if nobody acts.
...
6. **Challenge:** identify the assumption that could reverse the recommendation, its switching threshold, and one plausible combination of unfavorable changes. State what information or staged commitment could address the uncertainty.
7. **Decide and document:** state the choice, reasons, best alternative forgone, implementation owner, safeguards, and any justified exception. If AI materially informed it, attach the [minimum AI-use record](#minimum-ai-use-record).
...
For a low-stakes reversible choice, use only **define → challenge → decide → review**. For a consequential, repeated, contested, difficult-to-reverse, or partly automated decision, complete all eight stages.

## Take it forward


Return to the hiring meeting that opened the book. A better process makes the reasons for the choice clearer, exposes disagreement before commitment, and preserves what the outcome can teach. Start with one recurring decision and build the conditions under which the next decision can improve.

The [final chapter](42-data-driven-decision-making.qmd#separate-prediction-and-valuation) asks what changes when machine assistance makes a forecast or draft analysis available within seconds. The estimate may improve while the decision remains poor because the target is wrong, an important alternative is missing, or nobody can challenge the recommended action. The process developed here gives us a way to test whether the tool improves the whole decision.
