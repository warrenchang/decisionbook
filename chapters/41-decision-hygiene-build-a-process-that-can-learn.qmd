...
This chapter brings the book’s tools together around that task. We will follow what needs to be protected before a discussion, what disagreement can reveal during it, and what a later review can establish from the original record.
...
### What to record before and after a decision {#what-a-decision-journal-is}
...
The matched rows in @tbl-35-decision-journal identify the decision, separate prediction from valuation, and require the later review to answer the same questions recorded before choosing. Append the review; never edit the original entry into agreement with what occurred.
...
![Part VII follows a practical reading route: find the obstacle, change the conditions, and check the results.](../figures/master-loop-part-7.svg){#fig-master-loop-finale fig-alt="Three reading questions connect finding the obstacle, changing the conditions, and checking the results. They organize Part VII's design and learning tools." .phone-fit}

The three questions in @fig-master-loop-finale offer a brief way back into the main workflow: identify the obstacle, choose a change, and check its effect. If a review leaves the next step unclear, use the prompts in @tbl-35-1 to locate the unanswered question.
...
Use the two levels of protection in @tbl-36-proportionality to scale the procedure to the stakes. A low-cost reversible choice may need a short note and a review date; a repeated hiring process needs stronger evidence and checks for unfair treatment. Remove a control when its burden exceeds its useful contribution.
