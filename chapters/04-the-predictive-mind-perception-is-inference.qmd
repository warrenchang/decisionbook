...
### Color belongs to a scene {#color-and-illumination-context}

Compare the isolated lens on the left with the upper lamp on the right in @fig-traffic-light-color-context. The lens artwork is identical and uses only shades of gray. Within the blue-green traffic scene, however, it may look warmer or reddish. Cover the surrounding scene, then reveal it again. Does the apparent color change even though the lens itself stays the same?

![The same gray lens artwork appears alone and as the upper lamp of an original, stylized traffic signal in a blue-green scene. Context may give the right lens a warmer or reddish appearance. The lens interior has identical neutral-gray color values in both placements.](../figures/traffic-light-color-context.svg){#fig-traffic-light-color-context fig-alt="Two panels show the same gray dotted lens. On the left it has a neutral-gray surround. On the right it is the upper lamp of a traffic signal, with yellow-green and turquoise lamps below, within a blue-green streetscape. The upper lens may appear warmer or reddish despite sharing the left lens's gray artwork." width=100%}

The broader problem is **color constancy**: an object's perceived color can remain relatively stable even when the light reaching the eye changes with illumination. A white shirt can still look white in warm indoor light or cool daylight. Visual adaptation and relationships across a scene contribute to this stability; information about illumination can help distinguish an object's properties from the light falling on it (Foster, 2011).

Interpreting a scene can also change color appearance. In a laboratory demonstration, changing the perceived folding direction of a card changed its apparent color in a way consistent with altered interpretation of light reflected between its surfaces (Bloj et al., 1999). Our traffic drawing combines surrounding colors, a suggestion of a blue-green cast, and a familiar signal arrangement.[^ch04-color-cues] **The same pixel values can produce different color experiences in different surroundings.**
...
[^ch04-color-cues]: The drawing combines illumination cues with color contrast and adaptation, processes discussed by Shevell and Kingdom (2008).
