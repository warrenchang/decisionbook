...
# The Predictive Mind {#the-predictive-mind-perception-is-inference}
...
Context can bring learned structure into use. But how does a person acquire the connection between visual signals and familiar objects in the first place? Restored sight provides an unusually clear view of that learning problem.
...
Jennings’s experience reveals why prior learning matters: seeing features is not yet recognizing their arrangement as an object. Together with the illusions, it defines the challenge. We need learned structure to interpret incomplete signals, and reliable new observations to correct an interpretation when that structure misleads us.

## Two meanings of prediction

These examples show why learned expectations matter. To explain how they work, we first need to clarify a term that can otherwise cause confusion.
...
When the body loses water, changes in fluid concentration and circulating volume help generate thirst. Yet a drink can relieve that feeling quickly, while absorption and the restoration of fluid balance are still under way. Feeling less thirsty does not mean that a water deficit has already been corrected throughout the body. In a small human experiment, thirst fell within minutes of drinking; even a much smaller amount reduced reported thirst, although its hormonal effects differed (Williams et al., 1989).
...
Context can also change how urgently a bodily signal is experienced. I sometimes notice this when I approach a toilet: a need that did not feel pressing moments earlier suddenly does. The setting has changed much faster than the bladder could have filled. A familiar opportunity to act may direct attention toward the sensation and activate a learned association between that place and urination.

There is evidence for such cue sensitivity. In a small study of 12 women with situational urgency incontinence, photographs of personally relevant situations changed bladder sensation in 11 participants; personalized cues could also provoke measurable bladder activity and leakage (Clarkson et al., 2020). These clinical findings support the influence of context, while leaving the precise mechanism of my everyday experience open.

The visual and bodily examples share a useful lesson: what we experience depends on incoming signals together with the context in which the nervous system interprets them. Their mechanisms need not be identical. The thirst experiments provide evidence of anticipatory regulation; the toilet example points to learned cues and attention. [Chapter 5](05-expectations-when-predictions-become-causes.qmd#illness-after-a-deadline) considers a more uncertain case: feeling ill only after a demanding period ends, and why its timing alone cannot establish an expectation effect.
...
::: {#generative-ai-and-predictive-processing .callout-note .research-lens icon=false collapse=true}
## Research Lens: an analogy with generative AI
...
### What does a language model predict? {.unlisted}
...
### A prompt supplies a setting for interpretation {.unlisted}
...
### Images and videos: generation is broader than next-token prediction {.unlisted}
...
### Change the question—and then seek evidence {.unlisted}
...
### Optional exercise: change the frame {.unlisted}
