# Habits: The Plan Is Often Not the Decision-Maker
...
- Explain how cue ecology, reinforcement, and reward prediction error build automaticity.
...
In Lally and colleagues' field study, modeled time to reach near-asymptotic automaticity varied widely—from 18 to 254 days, with a median of 66 days among fitted participants (Lally et al., 2010). The numbers are not a new rule. They show why “a habit takes twenty-one days” is the wrong question. A simple action in a stable setting and a demanding action in a shifting setting do not have the same learning curve. Missing one opportunity did not by itself destroy the trajectory; repeated cue-linked practice mattered more than a perfect streak.

![Habit automaticity can rise along different gradual, leveling trajectories; a missed opportunity does not automatically reset learning.](../figures/habit-formation-curve.svg){#fig-habit-formation-curve fig-alt="A schematic chart plots automaticity against repeated cue-linked opportunities. Three different curves rise and level at different rates. A shaded missed occasion interrupts the horizontal sequence but does not reset any curve." width=86%}
...
This boundary changes coaching. A streak can make progress visible, but one broken streak should not become evidence that the learner has returned to zero. Measure the association becoming easier, faster, and more reliably cued—not only consecutive calendar marks.
...
- A habit is a learned cue-response pattern. The decision loop still exists functionally, but prediction and value have been compressed into an automatic response that the context can trigger before deliberation fully arrives.
- A cue is often an ecology of physical, temporal, social, digital, internal, and historical signals.
- Reward prediction errors update what cues and actions are expected to deliver; intermittent rewards and immediate relief can maintain costly loops.
- A useful redesign first asks what local problem the behavior solves and which cue configuration recruits it.

## Study and practice

1. Why is repeated frequency insufficient to define a habit?

2. How would you distinguish goal-directed and habitual control in one real behavior?

3. How do reward prediction error and delayed cost help explain a habit that persists without much enjoyment?

::: {.callout-tip .activity icon=false}
## Six-Minute Habit Autopsy

Choose a mild, private repeated behavior. For two minutes, reconstruct the cue ecology. For two minutes, name the response tendency, action, and immediate outcome—pleasure, progress, relief, or cost. For two minutes, state what the loop learned and what delayed consequence it ignored. Do not propose a solution until you can state the local problem the habit solves.
