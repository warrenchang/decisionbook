...
Accessibility changes which meaning is ready; familiarity and fluency change how easy that meaning feels. Both can support understanding, but neither is evidence by itself.
:::
...
## Affect can travel with the context

Accessibility can carry feeling as well as meaning. In affective-priming tasks, a positive or negative cue can facilitate evaluation of a target with the same valence (Fazio et al., 1986). Very brief affective cues have also shifted immediate evaluations of neutral targets under specific laboratory conditions (Murphy & Zajonc, 1993).

This does not mean a pleasant room or trusted logo remotely controls choice. It means an evaluation can borrow affect from what preceded or surrounded it. A rushed interviewer may experience the next candidate less favorably. A product displayed with friendship, adventure, or status can acquire some of those associations. A harsh opening in a difficult conversation can make the next ambiguous sentence feel hostile.

The valuation chapter separated **integral emotion**, which concerns the option, from **incidental emotion**, which arrives from elsewhere. Apply the same separation here. Ask: What feeling was already active? What produced it? Would the evaluation survive a different order, room, or prior event? When stakes are high, independent ratings, counterbalancing, breaks, or delayed review can reduce accidental carryover.
...
Masked-priming paradigms present a cue briefly and interfere with conscious report. They are valuable for studying how much processing can occur without ordinary awareness (Forster & Davis, 1984; Marcel, 1983). They do not show that hidden messages can implant complex goals, override preferences, or force purchases. Effects are generally narrow, task-dependent, and stronger when a cue connects with an existing state or goal. Awareness also matters ethically: a visible reminder aligned with a person's goal is different from concealed influence designed to defeat reflection.
:::
