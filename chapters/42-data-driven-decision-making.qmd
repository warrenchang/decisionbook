...
## Opening puzzle: why did better predictions provoke opposition? {#opening-puzzle-where-should-the-city-dig}
...
The [previous chapter](41-decision-hygiene-build-a-process-that-can-learn.qmd#ai-accountable-judgment) provides a process for separating estimates from valuations and learning from results. This chapter puts that distinction under pressure. As AI makes prediction cheaper, the question becomes how to turn its estimates into actions whose objectives, trade-offs, and authority people can examine and challenge.
...
The fall in cost is striking. Stanford's 2025 AI Index reports that the inference price of achieving approximately GPT-3.5-level performance on the MMLU benchmark fell by more than 280-fold between November 2022 and October 2024 (Stanford Institute for Human-Centered Artificial Intelligence, 2025).
...
The 2018 technical paper compared an actual unnecessary-visit rate of 18.8 percent with 2.0 percent under a proposed procedure in a **backtesting simulation** (Abernethy et al., 2018). The later author report documents the policy reversal introduced in the opening: in 2018, the new contractor's excavation choices largely bypassed high-risk areas, and the hazardous-line hit rate fell to 15 percent. A 2019 legal settlement required the city to return to model-guided prioritization, after which the hit rate rose again (Webb et al., 2020).
...
The following decisions determine how the map becomes a programme:
...
Suppose the city asks a team to “use AI” to improve pipe replacement. One member proposes a risk model, another a chatbot to answer residents' questions, and a third a scheduling tool for crews. Each addresses a different obstacle; choosing the most impressive demonstration could leave the main obstacle untouched. Begin by identifying the work the decision needs. Predictive models, language models, retrieval systems, and optimizers can then supply different parts of the workflow.
...
A model that reasons well through a clinical case appears to offer physicians a useful second opinion. Giving doctors access should then improve their answers—but will that advantage survive the way they actually consult and use the tool? In a randomized study, 50 physicians solved clinical vignettes with or without access to an LLM. Access did not significantly improve diagnostic reasoning: the adjusted difference was 2 percentage points, with a 95 percent confidence interval from −4 to 8 (Goh et al., 2024).
...
The design goal is therefore accountable human control with well-tested machine assistance.
...
Record the number of cases underlying each result and the uncertainty in estimated performance. [Chapter 15](15-samples-randomness-regression-and-calibration.qmd#calibration-makes-confidence-answerable) develops the discipline of calibration and learning from samples.
...
Complete the following canvas before selecting a product or writing a prompt.
...
This prompt structures a checkable output. Verify the supporting passages and important calculations before using the table.
