...
![A persuasive story begins in the audience’s current model, creates meaningful trouble, and resolves in an evidence-supported new model and action.](../figures/story-update.svg){#fig-story-update fig-alt="Five-step story sequence: current story, but or trouble, protagonist conflict, new model, therefore evidence and action."}
...
Some practitioners visualize the same movement as ordinary world → disrupted or “special” world → changed return. AND establishes the familiar context, BUT crosses into a problem that the old model cannot absorb, and THEREFORE returns with a revised explanation or course of action. This is a useful spatial metaphor for drafting, not evidence that every persuasive message or historical speech follows a universal monomyth.
...
ABT works best with one governing BUT. A weak BUT merely changes topic: “Customers use digital tools, BUT our product uses AI.” A strong BUT exposes a consequential failure in the current model: the tools are present, yet a decision that matters is still being made badly for an identifiable reason. Test the tension before drafting the THEREFORE: Is it true? Does this audience care? Does the proposed response become necessary rather than merely possible?

Randy Olson (2015) popularized ABT as a practitioner narrative framework. It is a drafting and diagnostic tool, not a psychological law, and a “narrative index” that counts connective words is at most a rough text heuristic. Historical speeches can nevertheless sharpen structural seeing. In paraphrase, Lincoln's Gettysburg Address moves from a nation founded on equality, through a war testing that proposition, to a duty carried by the living. Kennedy's inaugural address moves from inherited purpose, through an altered and dangerous world, to civic service. Roosevelt, Mandela, Churchill, and Reagan likewise move from shared ground, through crisis or contradiction, toward action. Martin Luther King Jr.'s 1963 address can be analyzed as promise, unfulfilled reality, and renewed collective action without reproducing its protected text. These are interpretive mappings: the speeches do not mechanically follow a three-word formula.

The lecture's two venture rewrites show how the same edit travels across domains. A smart-window pitch becomes intelligible when it begins with the shared goal of bright, comfortable, energy-efficient buildings, then reveals the conflict that large windows also add solar heat, and only then introduces heat-responsive glazing as a candidate mechanism. A diagnostic-test pitch becomes stronger when it begins with a familiar, fast, inexpensive lateral-flow test, then names the consequential limitation—some important cases require slower confirmation because sensitivity is limited—and only then presents signal amplification and its evidence. In both examples, the rewrite removes extra BUTs, places the problem first, and moves quantitative performance claims into the reasons-to-believe section. Any real company should verify those claims before publication; here the examples teach sequence, not product endorsement.

::: {.callout-tip .activity icon=false}
## Find the governing BUT

Choose a business idea, policy, behavior change, research claim, or course project. Draft “___, AND ___, BUT ___, THEREFORE ___.” Use one governing contradiction. Then test it: does the BUT reveal a real pain or prediction failure; would this audience care; and does the THEREFORE follow causally? Revise a feature-first pitch into a problem–mechanism–evidence pitch, then label every unsupported claim.
:::
...
## A vivid case cannot establish scale
...
Take one abstract claim from the course. Write a protagonist, goal, obstacle, stakes, choice, and turning point that make the mechanism visible. Then add the statistic that shows scale and representativeness.  
