...
- Diagnose coordination, conflict, mixed motives, and anti-coordination.
- Represent a strategic situation using players, actions, payoffs, information, and timing.
- Find best responses and Nash equilibria in simple games.
- Distinguish equilibrium from efficiency, fairness, prediction, and advice.
- Explain how focal points, communication, commitment, and institutions change strategic outcomes.
- Recognize competition neglect and escalation before entering the trap.
...
A **best response** is an action that gives a player the highest payoff for a specified action by the other player. A **Nash equilibrium** is a set of actions—one for each player—such that no player can gain by changing their action alone (Nash, 1950).
...
Behavioral game theory begins where the last column becomes unavoidable. The next chapter asks how real people form beliefs and how many steps of strategic reasoning they use.
...
## The 50-kr auction: competition neglect becomes escalation
...
- Strategic interdependence means that the value of an action depends on others' actions and reactions.
- A game specifies players, actions, payoffs, information, and timing; changing any element can change the prediction.
- Nash equilibrium is stability against unilateral deviation, not a guarantee of efficiency, fairness, or empirical accuracy.
- Focal points coordinate through shared expectations, but shared salience is group- and context-dependent.
- Multiple equilibria create a selection problem; strategic uncertainty can make a lower-payoff outcome safer.
- Commitment can coordinate expectations, but inflexibility can also create collision and escalation.
- Competition neglect begins before entry; a stop rule should evaluate future value rather than defend sunk cost.

## Study and practice

1. Give one example each of coordination, conflict, mixed motives, and anti-coordination.
2. In the price-war matrix, identify every best response and explain why low–low is not jointly best for the firms.
3. Why is a focal point different from a payoff-dominant equilibrium?
4. In the stag hunt, what belief about the other player makes Stag optimal?
5. Describe a commitment that improves coordination and one that increases escalation risk.
6. Name one outsider omitted from a familiar two-player game and explain how including them changes welfare.

::: {.callout-tip .activity icon=false}
## Activity: redraw a real strategic problem

Choose one live problem involving at least two decision-makers. Write the players, actions, payoffs, information, and timing. Classify the structure. Draw one baseline payoff table, identify best responses, and state every equilibrium. Then change exactly one feature—communication, timing, monitoring, commitment, rotation, or exit—and predict how behavior should change.

EXPERIENCE – EXPLAIN – APPLY (MAXIMUM 150 WORDS)  
Experience: describe the interaction without assigning personalities. Explain: identify the game and one uncertain assumption. Apply: propose one rule change, one prediction, and one ethical boundary.
:::
