# Building an Evidence-Aligned Story
...
![An ethical persuasive story braids human meaning with base rates, mechanisms, comparisons, and uncertainty.](../figures/story-evidence-braid.svg){#fig-story-evidence-braid fig-alt="Two horizontal strands, story and evidence, cross and converge on informed judgment."}
...
: A bounded SUCCES editing checklist {#tbl-27-succes}
...
The DPN materials also introduced Raymond Loewy's design principle MAYA: most advanced yet acceptable. The same principle applies to persuasive stories. A story that is too familiar may be boring because it creates no update. A story that is too strange may be rejected because the audience cannot enter it. Good persuasion often combines familiarity with surprise. It starts inside the audience's current model and then introduces a discrepancy that is close enough to understand but strong enough to matter.
...
## Watch language turn into a model

Watch the [“horse-sized syringe” debate excerpt](https://youtu.be/H1JFGWBAC5c) and the [“Anna and her grandfather” excerpt from *Borgen*](https://youtu.be/ksH2zT7I6NM). The clips are copyrighted examples, so analyze the linked originals rather than reproducing frames. For each, identify the concrete image or protagonist, the causal model it invites, the emotion and action it makes available, and the base rate or comparison evidence still needed. A memorable metaphor may help organize a claim; meta-analytic evidence suggests its average persuasive advantage is small and context-dependent (Sopory & Dillard, 2002).
:::
...
![Five motives can make a truthful message worth passing on: self-expression, connection, usefulness, competence, and collective change.](../figures/message-sharing-value.svg){#fig-message-sharing-value fig-alt="A central circle asks whether sharing a message would create value. Five evenly spaced boxes name self-expression, connection, usefulness, status or competence, and collective change as possible motives."}
...
: The value people may find in sharing a message {#tbl-27-sharing}
...
## Story structure cannot rescue weak evidence

Story frameworks are practical writing tools, not scientific laws. A memorable structure cannot rescue false evidence, a hidden conflict of interest, or a recommendation that removes meaningful choice.
:::
...
2. How can narrative be integrated with an evidence ladder without turning an anecdote into proof?

3. How should a story adapt to identity, resistance, and context while keeping disagreement possible?

::: {.callout-tip .activity icon=false}
## Persuasion design studio

Choose one research claim, policy, product, leadership request, negotiation move, or behavior change. Produce a 3–5 minute video or live message for one specific audience and one desired response.

1. Complete a six-box canvas: audience and current model; meaningful mismatch; proposed model or opportunity; evidence and uncertainty; ownership/stickiness/shareability; specific ask and genuine path to “no.”

2. Draft a 60–90 second spine with ABT or STORY. Add one identity/autonomy/consistency principle and one memory/shareability principle. State the ethical boundary for each.

3. Complete the story canvas: protected identity; protagonist; goal; stakes; obstacle; misbelief; turning point; evidence; and change. Begin near the disruption, end with one clear implication, and braid case → pattern → mechanism → solution → realistic resolution.

4. Submit a design note of no more than 200 words naming the audience, starting model, barrier, framework, evidence, uncertainty, persuasive principles, ask, and one manipulation risk you redesigned.

Peer feedback diagnoses the mechanism, not the presenter's style: What was the starting model? What mismatch required an update? What new model or action was offered? What made it credible, self-endorsable, memorable, or shareable? Is the next step explicit, and is refusal real? Give one diagnosis and one revision—not five preferences.
:::
