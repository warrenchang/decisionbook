...
::: {#chapter-40-start .chapter-epigraph}
> “We shape our buildings, and afterwards our buildings shape us.”
>
> — Winston Churchill, [speech on rebuilding the House of Commons](https://heritagecollections.parliament.uk/exhibits/rebuilding/), 28 October 1943
:::

[]{#opening-puzzle-the-form-that-already-chose}Imagine enrolling in a retirement plan. The contribution field already says 6 percent, a recommended fund sits first, and the opt-out link is in smaller type. You can choose another arrangement, but doing so requires more attention than accepting the form as it stands.
...
Menus, defaults, forms, and feedback shape how people choose. A useful design helps them understand consequences and act on informed goals; a harmful one profits from confusion or makes refusal difficult. Evaluate the whole path, including what happens when someone tries to change their mind.
:::
...
The choice among the instruments in @tbl-35-policy-tools depends on the obstacle. A reminder may help someone who forgets an appointment; an unaffordable service calls for a response to cost. Likewise, a clearer menu cannot supply a useful option that the institution never offered.

## The plausible path: where architecture enters the loop

![Five questions locate possible problems in the objective, menu, understanding, action, and feedback.](../figures/choice-architecture.svg){#fig-choice-architecture fig-alt="Five panels ask whose goal is served, which options and defaults are offered, whether outcomes can be compared, how friction affects action, and whether people can see results, correct errors, or leave." .phone-fit}

The five questions in @fig-choice-architecture locate the barrier a design must address. Trace the proposal from its objective to what the person can choose, understand, do, and correct:
...
A menu appears to give people freedom by laying out their choices. Yet a perfectly clear menu can still make the decision worse if it hides a useful combination, makes delay invisible, or omits the option of gathering more information. The first design task is therefore to construct the option set before polishing its presentation. Categories and order then shape comparison: people interpret an option relative to what appears beside it.

**Choice overload** occurs when enlarging a menu makes choosing harder in ways that reduce decision quality or satisfaction, or increase deferral. More options can also increase fit and autonomy, so the direction needs testing. At a supermarket tasting booth, Iyengar and Lepper (2000) alternated displays of 6 and 24 jams. The larger display attracted more passers-by, but among those who stopped, about 30% subsequently bought jam after the smaller display, compared with about 3% after the larger one. The study changed the tasting display, while buyers ultimately chose from the store’s full shelf. Scheibehenne and colleagues (2010) found a virtually zero average effect across 50 experiments, with substantial variation. Chernev and colleagues (2015) subsequently identified conditions associated with greater overload: uncertain preferences, complex comparisons, difficult tasks, and goals that favor minimizing effort. The useful diagnosis concerns the demands of this menu for this chooser.

A useful response is to structure comparison. Offer filters or an explainable shortlist, preserve access to the fuller menu, and make a legitimate option to wait or seek advice visible. Removing an apparent duplicate is sensible only after checking whether it serves a distinct need. Disclose why a featured option is prominent, especially when sponsorship plays a role.
...
The [amusement-park story in Chapter 12](12-framing-when-the-same-facts-become-different-decisions.qmd#personal-story-leaving-the-park) shows how a question can offer a small choice within a fixed boundary. From the perspective of choice architecture, the important feature is the **constrained menu**: the child chooses when to leave, while departure itself is settled. The same wording needs more care when the supposedly settled decision is still open.

::: {.callout-note .personal-example icon=false}
### From my experience: one egg or two? {#offer-a-clear-choice-within-honest-limits}

At breakfast, I might ask my daughter, “Would you like one egg or two?” rather than “Do you want eggs?” The question gives her a choice about quantity, but it also assumes that she wants an egg. If eggs are optional, “no egg” needs to remain an easy, respected answer. A small choice supports agency when it leaves room for what she actually wants.
:::
...
Make the comparison reconstructable: use the same time horizon and denominator, show total cost alongside installments, and distinguish an estimate from a guarantee. For probabilities, natural frequencies and absolute risks can help readers see quantities that relative percentages obscure. [Chapter 14](14-base-rates-conditional-probability-and-bayesian-updating.qmd#chapter-14-start) develops those formats.

Metrics are frames. [Chapter 12](12-framing-when-the-same-facts-become-different-decisions.qmd#chapter-12-start) showed why equivalent descriptions can change choice; architecture determines which description becomes the default view. “Save €40 per month” and “save €480 per year” are equivalent arithmetically but not psychologically. A fair design does not pretend framing can be eliminated; it uses symmetric, decision-relevant representations and makes important alternative frames available.

Even familiar units can map poorly. The curve in @fig-mpg-fuel-use shows that miles per gallon is nonlinear in fuel consumed: an improvement from 10 to 20 MPG saves much more fuel over a fixed distance than an improvement from 30 to 40 MPG. Gallons per hundred miles makes that consequence easier to compare (Larrick & Soll, 2008). The design question is not which unit looks intuitive, but which one reveals the welfare-relevant difference.

![Over a fixed 100-mile trip, fuel use equals 100 divided by MPG. Moving from 10 to 20 MPG lowers use from 10 to 5 US gallons, saving 5 gallons. Moving from 30 to 40 MPG lowers use from 3.33 to 2.50 gallons, saving 0.83 gallon—exactly one-sixth as much. The conversion makes the fuel savings visible (Larrick & Soll, 2008).](../figures/mpg-fuel-use.svg){#fig-mpg-fuel-use fig-alt="A reciprocal curve plots US gallons used over 100 miles against miles per gallon. The highlighted 10-to-20-MPG segment falls from 10 to 5 gallons, while the 30-to-40-MPG segment falls only from 3.33 to 2.50 gallons. Comparison cards show that the first change saves 5 gallons and the second saves five-sixths of a gallon, so the first saves exactly six times as much fuel." width="100%"}

The same principle applies to mental accounting (Thaler, 1985). A budget category can help someone control spending while hiding the total cost across accounts. Show both the local choice and its effect on the whole budget, as discussed in [Chapter 19](20-mental-accounting-money-is-fungible-minds-label-it.qmd#chapter-20-start).
...
Simplification must preserve meaning. Shortening a form helps only if the remaining questions still protect eligibility, safety, and informed choice. The paired remotes in @fig-choice-architecture-simplified-remote make this condition visible: hiding controls helps only if the controls needed for the task remain accessible. Likewise, removing text is not the same as reducing cognitive work; sometimes a comparison table or worked example adds words while making the decision easier.

![Masking seldom-used controls simplifies a remote. Keep the controls needed for the user's actual tasks.](../figures/choice-architecture-simplified-remote.png){#fig-choice-architecture-simplified-remote fig-alt="Two generic black remote controls lie side by side. The left has many small controls; most of the right remote is covered with removable white tape, leaving only a power button, navigation ring, and a few large controls exposed." width="68%"}
...
Forgetting is one possible barrier; planning is another. A reminder can include a concrete opportunity to act, such as choosing a date and time, while making rescheduling easy. If the barrier is an unattractive immediate experience, the [temptation-bundling example in Chapter 39](39-behavior-design-make-the-better-action-easier.qmd#temptation-bundling) illustrates a different mechanism: link the activity to an immediate reward (Milkman et al., 2014). Urgency should reflect a real constraint rather than pressure the person to decide before inspecting the terms.

A planning field has direct experimental support: prompting employees to write the date and time of a flu clinic increased vaccination relative to a reminder without that field (Milkman et al., 2011). Commitment devices address a different problem by letting a person constrain a predictable future choice; because they may impose costs, they are not always nudges in the strict sense. Save More Tomorrow illustrates a hybrid: workers committed part of future salary increases to saving, combining advance commitment, timing, and inertia (Thaler & Benartzi, 2004).
...
[]{#an-arrow-that-changed-the-action-path}

An input can be irrelevant to an option's objective consequences and still be causally relevant to the decision process. In 2012, Google added a clickable right-pointing arrow while also changing font size, spacing, and text layout in advertisements on its Display Network. The hotel offer did not become better merely because an arrow appeared, but the route from noticing the offer to clicking it became more prominent. Google reported higher clicks after the bundled redesign (Land & Yuan, 2012).[^ch40-bundled-redesign]

The arrow in the illustrative comparison in @fig-digital-arrow-affordance makes the next action easier to recognize without changing the offer. Whether the increased clicking helped users requires a further comparison with outcomes they value, such as finding a suitable service.

![An illustrative action cue within a bundled interface redesign. These generic hotel advertisements show how an arrow can make the next action prominent; they are not the original test stimuli. Google reported higher clicks after combining the arrow with typography, spacing, and layout changes, so the comparison does not isolate the arrow’s effect.](../figures/digital-arrow-affordance.svg){#fig-digital-arrow-affordance fig-alt="Two generic hotel advertisements show the same offer. The second contains a prominent right-pointing arrow." width="100%" .phone-fit}
...
For these digital paths, a technically available option may still be difficult to exercise. A cancellation route that requires exceptional time or persistence provides little practical freedom to leave. Use @tbl-35-ethics to examine both what the design offers and what using it actually requires, including the effort needed to refuse or correct a choice:
...
Scale is a separate test. Quantitative reviews find substantial variation across nudge designs, and trials run at scale by public nudge units have produced more modest average effects than prominent academic studies might lead designers to expect (Hummel & Maedche, 2019; DellaVigna & Linos, 2022). DellaVigna and Linos found average increases in take-up of 1.4 percentage points in the nudge-unit trials and 8.7 points in the academic-journal comparison.[^ch40-scale-comparison] Evaluate the design in the setting where it will operate.
...
[^ch40-bundled-redesign]: The public report gave no effect size and did not isolate the arrow from the other changes. It measured clicks rather than profit or user welfare.

[^ch40-scale-comparison]: The two averages come from different collections of interventions, rather than a randomized comparison of the same intervention at different scales.
