...
![Five questions locate possible problems in the objective, menu, understanding, action, and feedback. This author's diagnostic map identifies places to inspect; it is not a universal sequence of psychological stages.](../figures/choice-architecture.svg){#fig-choice-architecture fig-alt="Five panels ask whose goal is served, what options are offered, whether consequences can be compared, what makes action easy or difficult, and whether mistakes can be corrected. A footer asks whether people can decline and report harm."}
...
![Over a fixed 100-mile trip, fuel use equals 100 divided by MPG. Moving from 10 to 20 MPG lowers use from 10 to 5 US gallons, saving 5 gallons. Moving from 30 to 40 MPG lowers use from 3.33 to 2.50 gallons, saving 0.83 gallon—exactly one-sixth as much. The curve is a mathematical conversion, not an estimated behavioral effect (Larrick & Soll, 2008).](../figures/mpg-fuel-use.svg){#fig-mpg-fuel-use fig-alt="A reciprocal curve plots US gallons used over 100 miles against miles per gallon. The highlighted 10-to-20-MPG segment falls from 10 to 5 gallons, while the 30-to-40-MPG segment falls only from 3.33 to 2.50 gallons. Comparison cards show that the first change saves 5 gallons and the second saves five-sixths of a gallon, so the first saves exactly six times as much fuel." width="100%"}

The same principle applies to mental accounting. A budget category can help someone control spending while hiding the total cost across accounts. Show both the local choice and its effect on the whole budget, as discussed in [Chapter 20](20-mental-accounting-money-is-fungible-minds-label-it.qmd).
...
![Masking seldom-used controls makes the simplification problem concrete. The goal is not the fewest possible buttons, but an interface in which the controls that remain match the user's real tasks. Hiding a necessary function can create a new error rather than remove friction.](../figures/choice-architecture-simplified-remote.png){#fig-choice-architecture-simplified-remote fig-alt="Two generic black remote controls lie side by side. The left has many small controls; most of the right remote is covered with removable white tape, leaving only a power button, navigation ring, and a few large controls exposed." width="68%"}
...
Forgetting is one possible barrier; planning is another. A reminder can include a concrete opportunity to act, such as choosing a date and time, while making rescheduling easy. Urgency should reflect a real constraint rather than pressure the person to decide before inspecting the terms.
...
An input can be irrelevant to an option's objective consequences and still be causally relevant to the decision process. In 2012, Google added a clickable right-pointing arrow while also changing font size, spacing, and text layout in advertisements on its Display Network. The hotel offer did not become better merely because an arrow appeared, but the route from noticing the offer to clicking it became more prominent. Google reported higher clicks after the bundled redesign (Land & Yuan, 2012).

The example is instructive only with its evidence boundary intact. The public report did not disclose an effect size or separately vary the arrow, so it cannot establish that the arrow alone caused the increase. Nor does a click establish profit, informed choice, or user welfare. The design lesson is narrower: action affordances belong inside the causal path, and the outcome chosen to evaluate them matters.

![The same generic hotel offer is shown before and after a bundled interface redesign. The second version makes the next action prominent with a right-pointing arrow. Google reported higher clicks after a redesign that combined the arrow with typography, spacing, and layout changes; its public report did not isolate the arrow's independent effect, disclose an effect size, or establish user welfare.](../figures/digital-arrow-affordance.svg){#fig-digital-arrow-affordance fig-alt="Two generic hotel advertisements show the same offer. The second contains a prominent right-pointing arrow. A result box reports higher clicks after Google's bundled redesign, and an evidence-boundary box states that the public report did not isolate the arrow, disclose an effect size, or establish profit or user welfare." width="100%"}
...
Behavioral effects are heterogeneous. An average effect may conceal strong benefit for one group, no effect for another, and harm for a third. Context, trust, baseline behavior, implementation quality, and the availability of good options all matter. Mertens et al. (2022) reported an average behavioral effect across diverse choice-architecture studies, but its magnitude and robustness to publication bias have been disputed. A reanalysis using different publication-bias adjustments found no clear average effect (Maier et al., 2022). These aggregate disagreements do not establish that every intervention works or that none does; they make evidence for the particular design and setting essential.

Scale is a separate test. Quantitative reviews find substantial variation across nudge designs, and trials run at scale by public nudge units have produced more modest average effects than prominent academic studies might lead designers to expect (Hummel & Maedche, 2019; DellaVigna & Linos, 2022). DellaVigna and Linos found average increases in take-up of 1.4 percentage points in the nudge-unit trials and 8.7 points in the academic-journal comparison. These are averages from different collections of interventions, not a randomized test of making the same intervention larger. The response is not “nudges work” or “nudges fail,” but diagnosis, comparison, and evidence in the setting where the design will operate.
