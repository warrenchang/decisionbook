...
Story is useful here not as performance theater but as a practical tool for ethical model updating. Use **STORY** as the primary construction workflow. Complete its five moves in order, then test the pivotal claims against evidence. AND–BUT–THEREFORE (ABT) is an optional compression test for a short message, not a second workflow that every message must complete.
...
- **Try this next:** draft with STORY, identify the pivotal claim, and strengthen it with evidence that tests that claim; use ABT only if a shorter version is needed.
