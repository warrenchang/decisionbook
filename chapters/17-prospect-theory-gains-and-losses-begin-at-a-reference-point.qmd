...
An outcome can feel like a gain or a loss depending on what it is compared with. Prospect theory explains how that reference point, sensitivity to gains and losses, and the influence of probabilities can jointly shape a risky choice. Each part must be examined; a preference reversal does not reveal its cause by itself.
:::
...
Expectations can also become reference points. A €10,000 bonus may feel like a gain if no bonus was expected and a loss if €15,000 was promised. Models of expectation-based reference dependence make this idea explicit (Kőszegi & Rabin, 2006). But an expectation is not automatically a right, and an analyst should not infer it only because the choice fits the model.
...
For commonly fitted parameter values below one, the curve is inverse-S shaped: small probabilities receive disproportionately high weight, while moderate and high probabilities receive lower weight than their numerical size would suggest. Tversky and Kahneman (1992) estimated separate weighting parameters of $\gamma=0.61$ for gains and $\delta=0.69$ for losses in their fitted model. Again, these are estimates from one specification and dataset. The formula requires a parameter range that keeps it increasing: very low positive values can make it nonmonotonic and yield invalid negative cumulative decision weights (Ingersoll, 2008).
...
Endowment effects, effort-based valuation, status quo bias, and narrow framing are often placed under the prospect-theory umbrella. They are useful extensions, but none is a direct readout of one parameter. The following cases show how to distinguish those explanations.
...
Successful creation may change value through competence, authorship, customization, or familiarity; effort justification and anticipated loss are further possibilities. The experiment establishes the pattern more directly than it separates these mechanisms.
...
Staying can also reflect switching costs, uncertainty, an inferred recommendation, inattention, anticipated regret, or a valued commitment. The default may be substantively best. A status quo preference therefore does not establish irrationality or loss aversion.
