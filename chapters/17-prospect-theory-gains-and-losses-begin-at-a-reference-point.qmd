...
Expected utility gives a benchmark for risky choice, and final outcomes still matter. But people often evaluate changes from a reference point, become less sensitive as gains or losses grow, react more strongly to losses in many settings, and transform probabilities into decision weights. Therefore prospect theory offers a disciplined descriptive model of these regularities—useful for prediction and diagnosis, but not a universal law, moral rule, or automatic explanation for every default or valuation gap.
:::
...
Prospect theory shows how **Interpret** and **Value** transform the same outcomes. A reference point codes final states as gains or losses, while decision weights change how stated probabilities enter choice.
:::
...
Endowment effects, effort-based valuation, status quo bias, and narrow framing are often placed under the prospect-theory umbrella. They are useful extensions, but none is a direct readout of one parameter. This lens preserves the evidence while keeping the core theory focused on reference dependence, diminishing sensitivity, probability weighting, and the fourfold pattern.
...
5. **Alternative:** What would the investor buy today if holding €70 cash?
...
Many decisions can be represented as having one or more reference points and a unit of evaluation, even when neither is stated. When reference dependence is proposed, state and test the reference rather than assuming one. Designers can influence choice by making a status quo, gain, loss, account, or time horizon more salient, but the operative reference remains an empirical question.
...
- **Use this when** the same risky outcome changes appeal across reference points, probability ranges, or gain–loss descriptions.
- **Do not infer** loss aversion whenever ownership, effort, attention, information, inertia, or switching costs can also explain the choice.
- **Try this next** by translating the options into final states and the complementary frame before recommending action.
