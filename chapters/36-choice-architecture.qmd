...
- Explain why there is no neutral way to present a choice.
- Distinguish nudges from boosts, incentives, mandates, and structural change.
- Diagnose how menus, defaults, mapping, friction, timing, norms, and feedback enter the decision loop.
- Choose an intervention that fits the actual barrier rather than the designer's favorite tool.
- Evaluate effectiveness, heterogeneity, welfare, transparency, and ease of exit.
...
Metrics are frames. “Save €40 per month” and “save €480 per year” are equivalent arithmetically but not psychologically. A fair design does not pretend framing can be eliminated; it uses symmetric, decision-relevant representations and makes important alternative frames available.
...
When the desirable behavior is not yet the majority behavior, a **dynamic norm** can truthfully describe change—“more households are reducing food waste”—without pretending that consensus already exists. Such messages have shifted sustainable choices in experiments (Sparkman & Walton, 2017). They still require a relevant trend, honest denominator, and testing; “more people” is not a license to manufacture momentum.
...
A university offers hundreds of courses. Removing options would reduce fit. Leaving an undifferentiated catalogue creates overload. A better architecture asks students about constraints and goals, shows a small explainable shortlist, maps each course to schedule and program consequences, warns about prerequisite conflicts, preserves the full catalogue, and makes the basis of each recommendation visible.

The system should not silently optimize for filling unpopular classes. If institutional capacity shapes the ranking, that objective should be governed and disclosed. Students need a correction path when the model does not know what they know about themselves.
...
### Worked example: a persuasive path with symmetric exit

Suppose a university wants students to attend an optional career workshop. A message that says “People serious about their future have already registered” turns identity and social comparison into pressure. A stronger path states the likely benefit and evidence, shows the time cost and what the session will contain, offers registration and refusal with comparable clarity, and provides a reminder only to students who ask for one.

Persuasion still occurs: the benefit is made salient and action is easy. But the student can inspect the claim, decline without stigma, and reverse the choice. The ethical test is not whether influence disappeared; it is whether the route supports an informed goal rather than harvesting compliance.

### Worked example: a negotiation agenda

An agenda is choice architecture. If price appears first and receives most of the time, negotiators may anchor on a distributive frame before discovering differences that create value. A redesigned agenda begins with purpose and implementation, lists all issues, invites additions, exchanges priorities before numbers where appropriate, considers packages rather than isolated concessions, and ends with responsibilities, contingencies, and review.

For a draft-heavy procurement negotiation, the redesign can go further: build a joint issue map before exchanging complete contracts; have each team privately rank importance and constraints; choose actively between issue-by-issue bargaining and packages; keep draft language visibly editable so the first wording does not become an implicit entitlement; and maintain a shared log of agreements, open questions, owners, and review dates. The process does not force agreement. It makes a better search and more reliable implementation possible.

## Key ideas

- There is no neutral choice presentation; every menu has an order, default, language, timing, and path.
- A nudge is only one instrument. Use boosts, incentives, mandates, or structural change when they fit the barrier better.
- Menu, default, mapping, friction, timing, social information, and feedback enter different points in the decision loop.
- Simplification should reduce unnecessary work without hiding material meaning.
- Effects vary across people and settings, so treat interventions as hypotheses and measure distribution as well as averages.
- Ethical design serves a defensible objective, makes important features recoverable, protects meaningful exit, and monitors harm.

## Study and practice

1. Find one default in a digital or institutional environment. What mechanisms might make it influential besides effort?
2. Choose a recurring failure and diagnose whether the barrier is attention, understanding, value, ability, trust, timing, habit, social meaning, or access.
3. Redesign a menu without simply removing choice. What did you group, order, explain, or preserve?
4. Identify one friction that should be removed and one that should remain because it protects against error.
5. Compare a nudge with a boost and a structural change for the same problem.
6. Apply the ethical audit to a recommendation system, subscription flow, or workplace process.

::: {.callout-tip .activity icon=false}
## Activity: architecture autopsy and redesign
