...
[]{#opening-puzzle-meet-me-in-odense}After an earthquake, a volunteer agrees to help you leave the city. Your call cuts off before you settle on a meeting place or time, and you cannot reconnect. You have never met and know little about each other's habits or preferences.

Your favorite café comes to mind, but a stranger has no reason to think of it. The volunteer faces the same difficulty: knowing where they would like to go does not tell them where to find you.

You both want the meeting to happen. The challenge is to choose a place and time that each can expect the other to choose.
...
Think first of a place that would be easy for you to reach. Its convenience matters, but it will not help you meet if the volunteer chooses somewhere else. You need a reason to expect the same destination, despite knowing little about one another.

A familiar public landmark may offer such a reason. Suppose the entrance to the main station is widely recognized and safe to reach. You might choose it because you expect a stranger to think of it too. A time such as noon could work in the same way. What makes the choice promising is the expectation that both people will recognize it; neither personal preference nor physical convenience is enough by itself.

The volunteer is making the same inference about you. They may pass over their own favorite place because they have no reason to expect you to know it. Each person's choice depends partly on what they think will occur to the other. Your uncertainty therefore concerns someone who is also trying to anticipate you.

Shared knowledge can help without revealing private preferences. You may both know a prominent landmark, a public announcement, or a widely used convention. Such cues provide a basis for expectations, although two people can still attach different significance to them. The next chapter examines how a particular answer comes to stand out to both parties.

The value of your destination depends on where the other person goes. Game theory calls the value of an outcome to a player a **payoff**. In this situation, finding the volunteer is what makes the choice worthwhile. The next exercise separates personal preference from this dependence on another person's choice more sharply.
...
For a player concerned only with the matching reward, the best response is to choose a face with the highest probability of being selected by the other person. Your own ranking of the faces supplies possible evidence about their choice, but it no longer directly determines the monetary payoff.
...
These numbers approximate where a player might aim; they are not a guaranteed winning sequence. Your own number influences the mean, and winning depends on how close the other numbers are to the target. Reasoning from an expected average becomes more useful in a large group, where your own effect on that average is small.
...
The meeting problem required finding a place and time a stranger could anticipate. The face task separated private taste from rewarded prediction, and the guessing game made you examine where other people's reasoning might stop. In each case, evaluating your own choice required a view of someone else's action.
...
@tbl-beauty-contest-groups reproduces selected guessing-game results from their Table II. The sample sizes count participants across the reported groups; they are not necessarily the number playing together in one group. The fitted $\tau$ values in this table were chosen to match the observed mean, so matching those means is not an independent prediction test.
...
Coricelli and Nagel (2009) compared play against human and computer opponents using behavioral and neural measures. Their findings associate strategic reasoning with differences in processing when the opponent changes. Such evidence can help discriminate between accounts, but brain activation does not make a chosen number a unique indicator of the player's thought process.
:::
