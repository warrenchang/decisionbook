...
The meeting problem has a shared reward and no opportunity to communicate. Allowing a message would change the problem; rewarding the two people for different destinations would change it again. A **game** is a mathematical representation that makes these features explicit. At minimum, it specifies:
...
[Behavioral Game Theory](24-behavioral-game-theory-equilibrium-is-a-benchmark-not-a-portrait.qmd#chapter-24-start) investigates the unresolved questions in the right-hand column of @tbl-equilibrium-boundary (Camerer, 2003): how do real people form beliefs, and how many steps of strategic reasoning do they use?

## Coordination: shared expectations can do the work

In the price-war table, each firm has the same best response regardless of its rival’s choice. The Odense meeting is different: a place works only if the other person chooses it too. Coordination games often have several equilibria, so knowing that an outcome would work does not tell us which one people will select.
...
Many claim pairs are feasible, and many can be sustained by expectations. The players need compatible claims, but they also disagree about how much each should receive. A focal equal split may help; entitlement, contribution, alternatives, procedure, and power can support other claims. The model holds the total fixed at 4. Actual negotiation can also search for new arrangements that increase what is available to share.

Increasing the total still leaves the distribution unresolved, and a Pareto improvement can leave an unfair division intact. Efficiency asks whether someone can be made better off without making another worse off. Fairness asks how benefits, burdens, voice, and procedure should be distributed. Neither criterion eliminates the other.

## First diagnose the strategic problem

Before choosing a move, identify the structure of dependence. The branches in @fig-strategic-situation-diagnostic are strategic archetypes, not mutually exclusive categories. Mixed motives combine cooperation and conflict; anti-coordination is a coordination family in which players benefit from taking different rather than matching roles. Let the specified payoffs determine which features coexist.
...
| --- | --- | --- | --- |
| Coordination through matching actions | Interests broadly align, but actions must match. | Miscoordination or incompatible standards. | What common signal or convention can both sides recognize? |
...
A clear agreement on a place and time would remove the central uncertainty in the Odense meeting problem. In a departmental conflict, agreement also has to cover training costs, the chosen standard, and what happens if it fails. In both cases, better advice begins by making the dependence visible.

::: {.callout-note .applied-module icon=false collapse=true}
## Applied module: market rules change the strategic game

Buyers and sellers may stand to gain from trade yet fail to find acceptable terms. Compare the rules in @tbl-market-institutions: each changes who can move, when they move, what they observe, and how a price is formed, with implications for whether the gains from trade are realized.
