...
Individual decisions can often be mapped without specifying another chooser’s move. But once outcomes depend on what others do, an action has no fixed value by itself: beliefs, information, timing, and the game each person thinks they are playing enter the decision. Therefore game theory changes the unit of analysis and supplies disciplined benchmarks for joint dependence—without assuming perfect calculation, desirable equilibrium, or that the analyst has correctly identified the perceived game.
:::
...
These compact games extend the core coordination diagnosis. They are useful when the application requires their particular payoff structure; readers can proceed directly to commitment if coordination and the stag hunt already supply the needed language.
