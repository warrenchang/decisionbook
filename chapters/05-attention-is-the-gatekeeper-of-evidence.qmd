# Attention Is the Gatekeeper of Evidence
...
The scale of the bottleneck is easy to underestimate. The adult brain is roughly 2% of body mass but accounts for about 20% of resting energy use. Difficult tasks do not multiply whole-brain consumption; they mainly redistribute activity within a system that is already working continuously. At the information level, Zheng and Meister (2025) estimate that human sensory systems collectively gather information on the order of $10^9$ bits per second, while deliberate behavioral output proceeds at roughly 10 bits per second. These are deliberately broad order-of-magnitude estimates across unlike channels, not a literal claim that the brain discards every other bit. Their importance is architectural: a vast, parallel sensory stream must be compressed into a very small, serial stream of report and action.

![A high-dimensional sensory stream narrows through attention into a much smaller stream of conscious report and action.](../figures/attention-bottleneck.svg){#fig-attention-bottleneck fig-alt="A wide box labeled sensory systems at approximately one billion bits per second passes through an attention funnel into a box labeled conscious report and action at approximately ten bits per second. A note explains that the numbers are order-of-magnitude estimates and that selected information alone can guide deliberate judgment."}
...
![Attention does not merely inspect evidence; it determines which information becomes evidence at all.](../figures/attention-filter.svg){#fig-attention-filter fig-alt="Many information streams narrow through an attention funnel into evidence for judgment."}
...
Audit one recent meeting or class. List what captured attention, what was probably missed, and one change to the agenda, role assignment, representation, or environment that would make the missing information visible.  
