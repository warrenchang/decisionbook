...
The hiring calculation became useful only after the applicants and possible results were organized into a common population. Without that structure, a percentage can answer a different question from the one the decision requires. Probability notation makes the same logic available for other decisions.
...
In the hiring example, $P(\text{positive screen}\mid\text{strong performance})$ asks what share of strong performers pass. The committee instead needs $P(\text{strong performance}\mid\text{positive screen})$, the share of those who pass who are strong performers. Everything after the vertical bar specifies the group we are counting within.
...
The **prior** $P(A)$ gives the starting probability, which is a prevalence in this screening example. The **likelihood** $P(B\mid A)$ asks how probable the evidence would be if $A$ held. The competing likelihood $P(B\mid A^c)$ asks how often the same evidence appears without $A$. The **posterior** is the probability after incorporating the evidence (Bayes, 1763).

For the committee, the formula is useful only if these inputs describe the applicants and performance standard it actually faces. Selection into the pool, changes in working conditions, and uncertainty about the test’s error rates all affect that judgment.


## Natural frequencies make Bayes visible

Knowing the formula does not guarantee that the relevant denominator stays visible. Translating percentages into people makes it easier to inspect who could have produced a positive result.

For a second hypothetical example, suppose 1 percent of a population has a disease. A test is positive for 90 percent of people who have it and for 9 percent of people who do not. A person tests positive. The tempting answer is 90 percent, but that is $P(+\mid D)$ rather than $P(D\mid +)$.
...
[Chapter 9](09-what-feels-likely-availability-affect-and-resemblance.qmd#when-category-fit-substitutes-for-probability-resemblance) showed why a coherent story can feel probable. Probability supplies a non-negotiable set relation:
