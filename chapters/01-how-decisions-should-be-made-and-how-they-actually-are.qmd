...
![A rational-choice benchmark begins with feasible alternatives, beliefs about consequences, and values over consequences. Together they support an overall comparison and the choice of a highest-ranked feasible option.](../figures/rational-benchmark-portrait.svg){#fig-rational-benchmark fig-alt="Three inputs—feasible alternatives, beliefs and probabilities, and preferences and values—support a consequence-based comparison and choice of a highest-valued feasible option."}

@fig-rational-benchmark deliberately uses the broader label **consequence-based value**. Expected utility is one formal representation for choice under risk; it is not the only way to express the benchmark.
...
Second, once beliefs and values have generated an overall ranking, **the choice rule is not another substantive input**. The rule is simply to choose a highest-ranked feasible alternative. Conditions such as completeness and transitivity concern whether preferences can be represented coherently; they are properties of the ranking. Under uncertainty, stronger representations require additional assumptions. [Appendix D](../appendices/appendix-d-rational-choice-and-decision-analysis.qmd) states those assumptions explicitly, and [*Risky Decision-Making*](16-risky-decision-making-a-probability-is-not-yet-a-feeling.qmd) develops expected utility, certainty equivalents, and risk premia.
...
Most importantly, the model is a **benchmark**, not yet a psychological portrait. It tells us what a coherent consequence-based choice would require once the alternatives, beliefs, and values have been specified. It does not tell us how those inputs enter the mind.
...
The experiment nevertheless illustrates why an option has no fixed value outside the model through which it is interpreted. The same venue can promise safety, belonging, novelty, or distinction depending on the person's active concerns. A thin model can represent this by changing values, but it does not predict when and how those values will change. [*Valuation*](05-valuation-how-options-become-worth-choosing.qmd#motives-change-the-decision-landscape) develops state-dependent value.
...
## A decision is a process, not a moment
...
The visible choice is therefore the endpoint of hidden work. A functional decision loop makes that work inspectable.

## A functional decision loop {#a-functional-decision-loop}

![The recursive decision loop. Context and information feed noticing and interpretation, option construction, prediction, valuation, choosing, commitment and action, and learning. Other minds, institutions, and designed environments can alter every function.](../figures/master-loop-portrait.svg){#fig-decision-loop-ch1 fig-alt="A recursive vertical loop from context and information through notice and interpret, construct options, predict, value, choose commit and act, and observe and learn. A shared influence rail shows that other minds, institutions, and designed environments can affect every function."}

The sequence in @fig-decision-loop-ch1 is functional, not rigid. Option construction, prediction, and valuation often iterate. A new forecast can make another option imaginable. Recognizing what matters can change the information sought. Feedback may arrive before an action is complete. A practiced response may compress several functions into a moment. Other people, institutions, and technologies can influence every connection.
