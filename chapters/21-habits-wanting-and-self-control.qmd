...
Intertemporal plans often fail at the moment of action, and repeated contexts can gradually transfer control from deliberation to learned cue-response patterns. But habit, wanting, and liking are not the same: a cue can pull behavior even when the outcome brings little pleasure, and an urge is not evidence of weak character. Therefore this chapter diagnoses the learned loop so self-control can redesign context and treat urges as predictions rather than commands or moral verdicts.
:::
...
![Habit automaticity can rise along different gradual, leveling trajectories. A missed opportunity adds no repetition and may pause the gain without resetting prior learning.](../figures/habit-formation-curve.svg){#fig-habit-formation-curve fig-alt="A schematic chart plots automaticity against scheduled cue occasions. Three different curves rise and level at different rates. Each curve is flat across one shaded missed occasion and resumes rising afterward, showing a pause in strengthening without a reset to zero." width=86%}
